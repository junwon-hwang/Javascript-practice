...
const traderMaxValue = traders
.map(user => user.value)
.reduce((user,n)=>user<n?n:user)

const traderINfo = traders
.filter(user => user.value === traderMaxValue)
.map(user => (`거래자 정보:(${user.trader.name}, ${user.trader.city}) ,거래액 :${user.value}`))
